...
import { StockDetail, StockDetails, DetailInStock } from './stock-detail';
...
export class StockDetailService {
  ...
  getDetailsInStock(): Observable<StockDetail[]> {
    return this.http.get<StockDetails>(this.stockDetailsUrl).pipe(
      map((res: StockDetails) => res.details)
    );
  ...
  getOrders(time_start: string, time_end: string): Observable<Order[]> {
    ...
    return this.http.get<Orders>(this.logsUrl, {params: params}).pipe(
      map((logs: Orders) => logs.logs)
    );
  }
